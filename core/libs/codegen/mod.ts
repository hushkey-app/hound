...
export async function generateTypes(
  ...
  if (jobs.length === 0) {
    console.warn(
      '[hound codegen] No defineJob exports found in:',
      resolvedJobDirs,
    );
    return;
  }
  ...
  const mapEntries = jobs
    .map(({ exportName, eventName }) =>
      `  '${eventName}': _Data<typeof ${exportName}>;`
    )
    .join('\n');
...
class ManagementJobsClient {
  ...
  async find(options?: { queue?: string; status?: string; limit?: number; offset?: number }): Promise<JobRecord[]> {
...
export class HoundClient {
  ...
  /**
   * Emit and wait for the job to finish. Resolves with the jobId on completion;
   * throws on job failure or wait timeout (server caps the wait at 120s).
   */
  async emitAndWait<K extends keyof HoundJobMap>(
    event: K,
    data: HoundJobMap[K],
    options?: BoundEmitOptions & { timeoutMs?: number },
  ): Promise<string> {
    const res = await fetch(\`\${this.#url}/emit/wait\`, {
      method: 'POST',
      headers: this.#headers,
      body: JSON.stringify({ event, data, options }),
    });
    ...
    if (res.status === 408) {
      throw new Error(\`[hound-client] emitAndWait timed out: \${body.error ?? ''}\`);
    }
    if (!res.ok) {
      throw new Error(\`[hound-client] emitAndWait failed: \${body.error ?? res.statusText}\`);
    }
